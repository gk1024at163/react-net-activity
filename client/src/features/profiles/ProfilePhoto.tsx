...
export default function ProfilePhoto() {
    ...
    const { photos, loadingPhotos, isCurrentUser,
        uploadPhoto, setMainPhoto, profile, deletePhoto
    } = useProfile(id);
    ...
    const handlePhotoUpload = (file: Blob) => {
        uploadPhoto.mutate(file, {
            onSuccess: () => {
                setEditMode(false);//上传成功后关闭编辑模式，会触发重新渲染，显示新的照片
            },
        });
    };
        ...
        <Box>
            ...
            {editMode ? (
                ...
                (
                    <>
                        {photos.length === 0 ? (
                            <Typography>No photod added jet</Typography>
                        ) : (
